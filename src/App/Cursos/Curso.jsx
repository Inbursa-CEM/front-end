...
const Curso = ({ idCurso, nombre, prioridad, estado, fecha, idUsuario }) => {
  ...
  const idAgente = idUsuario;
  const [, , , eliminarCurso] = useContext(contextoGlobal);
  ...
  const Eliminar = () => {
    console.log(idCurso);
    fetch(
      `http://${process.env.REACT_APP_BACK_HOST}:8080/curso/desasignar`,
      options
    )
  ...
  const Guardar = () => {
    ...
    fetch(
      `http://${process.env.REACT_APP_BACK_HOST}:8080/curso/modificarAsignacion`,
      options2
    )
      .then((response) => response.json())
      .then((data) => {
